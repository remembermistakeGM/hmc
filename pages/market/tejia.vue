<template>
  <view class="tejia">
	...
	<view class="mj_cell yh" v-for="(item,index) in yhList" :key="index"> 
		<view class="mj_cell_text">特价商品：</view>
		<view class="mj_cell_input">
			<view class='tj_goods'>{{item.goodsname}} ￥{{item.num}}</view>
			<text class="texttj">特价价格</text>
			<input type="text" :value="item.num2" placeholder="请输入特价价格" />
			<text>元</text>
		</view>	
		<view class='closebtn'><image @click="reduce" :data-index="index" src="../../static/close-circle.svg"></image></view>
	</view>
		<view class="mj_cell">
			<button class="mj_cell_btn"  @click="plus">添加特价商品</button>
	</view>
</view>
	<!-- 选择商品弹窗 -->
<main-mask :maskStatus="isShowMask" :mWidth="1500" :mHeight="900"  @onClose="onCloseMask">
	<template v-slot:content> 
	  <view class="mask_title">选择特价商品</view>
    ...
    return {
	 ...
	 date1: currentDate1,
		...
		isShowMask:false,
			// 调货入库
			goodslist:[
              {id: '458',num:'1', goodsname: '波士顿大龙虾', },
              {id: '458',num:'10', goodsname: '波士顿大龙虾', },
              {id: '458',num:'01', goodsname: '波士顿大龙虾', },
              {id: '458',num:'1', goodsname: '波士顿大龙虾', },
              {id: '458',num:'1', goodsname: '波士顿大龙虾', },
              {id: '458',num:'1', goodsname: '波士顿大龙虾', },
              {id: '458',num:'1', goodsname: '波士顿大龙虾', },
...
computed: {
  ...
  onSelectgoods(e){
	...
	this.yhList.push(this.goodslist[index])
